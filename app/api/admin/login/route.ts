...
export async function POST(request: Request) {
  ...
  const attempt = await db.prepare("SELECT count,until FROM attempts WHERE key='login'").first<{count:number;until:number}>();
  ...
  if (!await verifyAdmin(db,value.email,value.password)) {
    const count = attempt && attempt.until>Date.now() ? attempt.count+1 : 1;
    await db.prepare("INSERT OR REPLACE INTO attempts(key,count,until) VALUES('login',?,?)").bind(count, attempt && attempt.until>Date.now() ? attempt.until : Date.now()+900_000).run();
    return Response.json({error:'電郵或密碼不正確'}, {status:401});
  }
  await db.prepare("DELETE FROM attempts WHERE key='login'").run();
  (await cookies()).set(sessionCookie,await newSession(db),{httpOnly:true,sameSite:'strict',secure:new URL(request.url).protocol==='https:',path:'/',maxAge:8*3600});
