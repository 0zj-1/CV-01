...
const empty=():ManagedProject=>({id:0,slug:'',title:'',description:'',cover:'',coverAlt:'',pdf:'',images:[],videos:[],placeholderNumber:'',placeholderLabel:'PROJECT IMAGE PLACEHOLDER',detail:{category:'',year:String(new Date().getFullYear()),headline:'',introduction:'',approach:''},published:false,placement:'more',order:0});
export default function Manager({initial}:{initial:ManagedProject[]}){
 ...
 async function api<T extends {error?:string}>(url:string,options:RequestInit){const r=await fetch(url,options);const data=await r.json() as T;if(!r.ok)throw Error(data.error||'操作失敗');return data;}
 async function upload(file:File|undefined,field:'cover'|'images'|'videos'|'pdf'){
  ...
  setBusy(true);setMessage('上傳中…');try{const form=new FormData();form.set('file',file);const result=await api<{url:string;type:string;error?:string}>('/api/admin/upload',{method:'POST',body:form});
   if(field==='pdf'?result.type!=='application/pdf':field==='videos'?!result.type.startsWith('video/'):!result.type.startsWith('image/'))throw Error('請選擇正確的圖片、影片或 PDF 類型');
   change(field==='pdf'?{pdf:result.url}:field==='cover'?{cover:result.url}:{[field]:[...work[field],result.url]});setMessage('素材已上傳，請按「儲存作品」套用。');
 ...
 <label>作品 PDF（每頁會依序顯示在原作品內容下方）<input type="file" accept="application/pdf,.pdf" onChange={e=>{void upload(e.target.files?.[0],'pdf');e.target.value='';}}/></label>
